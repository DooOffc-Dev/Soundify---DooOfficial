// ============================================================
// DOOHIGH AI V1.0.0 – SCRIPT.JS (FULL FIX + PERSISTENT PLAYER)
// ============================================================
...
// --- 3. PERSISTENT PLAYER (FLOATING MINI + FULLSCREEN) ---
function injectPersistentPlayerHTML() {
    // Cek apakah udah ada, kalo udah skip
    if (document.getElementById('persistentMiniPlayer')) return;
    
    // Mini Player (nempel di bawah layar)
    const miniHTML = `
    ...
    <div id="fullscreenPlayerModal">
        <button class="fs-close" onclick="closeFullscreenPlayer()">✕</button>
    ...
    // Event klik mini player buka fullscreen
    document.addEventListener('click', function(e) {
        const mini = document.getElementById('persistentMiniPlayer');
...
function showPersistentPlayer(track) {
    ...
    if (!mini) {
        injectPersistentPlayerHTML();
        setTimeout(() => showPersistentPlayer(track), 100);
        return;
    }
    
    const coverImg = track.cover || track.img || 'https://placehold.co/48x48/282828/FFFFFF?text=Music';
    ...
    persistentTrack.queue = track.queue || [];
    persistentTrack.queueIndex = track.queueIndex || 0;
    
    // Buat audio object
    if (persistentTrack.audio) {
        persistentTrack.audio.pause();
        persistentTrack.audio = null;
    }
    
    // Kalo ada URL audio langsung, pake itu. Kalo ga, pake YouTube
    if (track.audioUrl) {
        persistentTrack.audio = new Audio(track.audioUrl);
        persistentTrack.audio.volume = 0.8;
        persistentTrack.audio.addEventListener('timeupdate', updatePersistentProgress);
        persistentTrack.audio.addEventListener('ended', function() {
            nextTrackPersistent();
        });
        persistentTrack.audio.addEventListener('loadedmetadata', function() {
            const dur = persistentTrack.audio.duration;
            if (!isNaN(dur)) {
        ...
        persistentTrack.audio.play().then(() => {
    ...
    if ('mediaSession' in navigator) {
        navigator.mediaSession.metadata = new MediaMetadata({
            title: track.title || 'Judul Lagu',
            artist: track.artist || 'Artis',
            artwork: [{ src: coverImg, sizes: '512x512', type: 'image/png' }]
        });
        navigator.mediaSession.setActionHandler('play', () => togglePlayPersistent());
        navigator.mediaSession.setActionHandler('pause', () => togglePlayPersistent());
        navigator.mediaSession.setActionHandler('previoustrack', () => prevTrackPersistent());
        navigator.mediaSession.setActionHandler('nexttrack', () => nextTrackPersistent());
    }
}
...
function togglePlayPersistent() {
    if (persistentTrack.audio) {
        if (persistentTrack.isPlaying) {
            ...
            persistentTrack.audio.play().catch(() => {});
        }
...
function updatePlayIcons(isPlaying) {
    ...
    const path = isPlaying ? '<path d="M6 4h4v16H6z M14 4h4v16h-4z"/>' : '<path d="M8 5v14l11-7z"/>';
...
function updatePersistentProgress() {
    if (!persistentTrack.audio) {
        // Kalo pake YouTube, ambil dari ytPlayer
    ...
    const current = persistentTrack.audio.currentTime;
    const duration = persistentTrack.audio.duration || 1;
    const progress = (current / duration) * 100;
...
function seekToPersistent(value) {
    if (persistentTrack.audio) {
        ...
        persistentTrack.audio.currentTime = seekTime;
    } else if (ytPlayer) {
        seekTo(value);
    }
}
...
function prevTrackPersistent() {
    showToast('Lagu sebelumnya (implement queue)');
    if (persistentTrack.queue && persistentTrack.queue.length > 0) {
        persistentTrack.queueIndex = (persistentTrack.queueIndex - 1 + persistentTrack.queue.length) % persistentTrack.queue.length;
        const prev = persistentTrack.queue[persistentTrack.queueIndex];
        if (prev) showPersistentPlayer(prev);
    }
}

function nextTrackPersistent() {
    showToast('Lagu berikutnya (implement queue)');
    if (persistentTrack.queue && persistentTrack.queue.length > 0) {
        persistentTrack.queueIndex = (persistentTrack.queueIndex + 1) % persistentTrack.queue.length;
        const next = persistentTrack.queue[persistentTrack.queueIndex];
        if (next) showPersistentPlayer(next);
    } else if (currentPlayContext && currentPlayContext.data) {
        // Ambil dari konteks
        const idx = currentPlayContext.data.findIndex(t => t.videoId === currentTrack?.videoId);
        if (idx !== -1 && idx + 1 < currentPlayContext.data.length) {
            const nextTrack = currentPlayContext.data[idx + 1];
            showPersistentPlayer(nextTrack);
        }
    }
}
...
// Override fungsi togglePlay bawaan biar sync sama persistent
const originalTogglePlay = window.togglePlay;
window.togglePlay = function() {
    if (persistentTrack.audio && persistentTrack.audio.src) {
        togglePlayPersistent();
    } else if (originalTogglePlay) {
        originalTogglePlay();
    }
};
...
function toggleShuffle() {
    ...
    const color = isShuffle ? '#1ed760' : 'var(--text-sub)';
...
function updateMediaSession() {
    if ('mediaSession' in navigator && currentTrack) {
        ...
        navigator.mediaSession.setActionHandler('previoustrack', function() { showToast('Lagu sebelumnya'); });
    }
}
...
// --- BIAR LAGU TETAP JALAN SAAT KELUAR APP / TAB ---
document.addEventListener('visibilitychange', function() {
    if (document.hidden) {
        // Tab disembunyikan, audio tetap jalan (default browser udah jalan)
        // Tapi kita update state aja
        console.log('App di background, musik tetap jalan!');
    } else {
        // Tab aktif lagi, update UI
        if (isPlaying) {
            // Re-sync mini player
            const mini = document.getElementById('persistentMiniPlayer');
            if (mini && currentTrack) {
...
// Request audio focus (buat PWA) - media session udah di set di atas

console.log('🔥 DooHIGH AI - Soundify Music ');